...
## Summary
Machine learning engineer at Amazon Prime Video operating production model-serving, deployment, and evaluation infrastructure for LLM and retrieval workloads at 1M+ users, owning model lifecycle, rollback, and reliability. First-author EMNLP 2025 researcher.
...
- Drove deployment reliability with service-level auto-rollback alarms and a functional deep health check, root-causing a grace-period race that cut intermittent failures to zero.
- Launched a migration of the assistant backend to a new Amazon Bedrock model with reserved-throughput overrides and A/B-gated prompt regression testing, owning the production model lifecycle.
...
- Delivered backend REST and gRPC services for Alexa voice integration serving 100K+ queries/day under a 300ms SLA, parallelizing downstream calls to improve P99 latency 3x.
...
- Delivered an auto-scaling AWS (EC2, ELB, SQS, S3) Flask REST system handling 1,000 concurrent requests with 30-second elastic scale-down for cost-efficient scaling.
## Education
...
ML: PyTorch, TensorFlow, scikit-learn, Koog, MLflow, XGBoost
Infra: Kubernetes, Docker, CI/CD, gRPC, Event-Driven, Distributed Systems
Cloud: AWS EC2, S3, SQS, Lambda, SageMaker, CloudWatch