...
## Summary
ML engineer at Amazon Prime Video building production retrieval, ranking, and evaluation systems at consumer scale. First-author EMNLP 2025 researcher; MS CS 3.9 with ML and cloud depth.
...
- Trained a LoRA fine-tuned query router with multilingual E5 embeddings on a SageMaker endpoint, improving relevance 5% and cutting latency 50%; adopted for production.
...
- Diagnosed model failures through an 11-experiment ablation study, fixing reasoning poisoning and dropping the error rate from 75% to 0% across repeated batches.
- Optimized query-time inference by reordering candidate filtering ahead of metadata fetch and bounding cache, cutting payload 75% and memory 40% under peak load.
- Launched embedding-based retrieval and ranking (FAISS, KNN, NDCG) serving 1M users at 0.5s latency, improving CTR 16% via offline and online evaluation.
...
ML: PyTorch, TensorFlow, scikit-learn, Koog, Fine-tuning (LoRA/PEFT), Model Evaluation, XGBoost
Retrieval: FAISS, Vector Search, RAG
Systems: Docker, Kubernetes, AWS SageMaker