...
## Summary
ML Engineer at Amazon Prime Video building large-model evaluation frameworks and LLM-as-judge systems at scale. First-author EMNLP 2025 researcher with hands-on LoRA fine-tuning and agent evaluation.
...
- Established a multi-turn LLM evaluation framework processing 50K+ conversations/day with LLM-as-judge and automated fact validation, increasing defect discovery 60%.
- Diagnosed anomalous model failures through an 11-experiment ablation study, root-causing data poisoning versus architecture and dropping error rate from 75% to 0% across 50-run batches.
- Trained a LoRA fine-tuned query router for a hybrid retrieval system, improving relevance 5% and cutting latency 50%; won internal hackathon and adopted for production.
- Rolled out LLM regression testing (PromptFoo) with a retry methodology that solved flaky tests, now used team-wide to catch quality regressions before production.
- Reworked a trust-and-safety filter from keyword to intent-based detection, closing a content-bypass gap, validated across 50-run regression batches with zero false negatives.
...
ML: PyTorch, TensorFlow, scikit-learn, Koog, LLM-as-Judge, Model Evaluation, LoRA/PEFT, NLP, Deep Learning
Eval: PromptFoo, A/B Testing, Experimental Design
Retrieval: FAISS, RAG, Agents